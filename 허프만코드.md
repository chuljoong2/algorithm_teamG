# 허프만 코드
허프만코드는 데이터를 압축할 때 사용하는 그리디 알고리즘 중 하나로
문자열에서 빈도수가 높은 문자는 짧은 비트로, 빈도수가 낮은 문자는 긴 비트로 표현한다.
...
이후 해시맵의 개수만큼 노드객체를 만들고 해시맵에 있는 정보를 객체에 넣는다.
```java		
